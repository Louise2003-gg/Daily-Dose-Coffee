...
export default function Intro({ onFinish }) {
  ...
  // phases: start → logoIn → tagline → exit → done
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#0e0a06]
    ...
    >
      ...
      {/* Horizontal lines */}
      <div
        className={`absolute left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#b8860b]/40 to-transparent
          transition-all duration-1000 delay-500
          ${phase === "start" ? "opacity-0 top-1/2" : "opacity-100 top-[38%]"}`}
      />
      <div
        className={`absolute left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#b8860b]/40 to-transparent
          transition-all duration-1000 delay-500
          ${phase === "start" ? "opacity-0 top-1/2" : "opacity-100 top-[62%]"}`}
      />
      ...
      >
        <p className="font-lobster text-[#b8860b] text-4xl tracking-widest">Daily Dose</p>
      </div>
      ...
      >
        <p className="text-white/50 text-sm tracking-[0.4em] uppercase font-light">
